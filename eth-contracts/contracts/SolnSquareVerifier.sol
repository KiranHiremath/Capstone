...
// TODO mapping to track unique solution
...
function addSolution(
            ...
            ) external
    {
        ...
        //Unique solution accross all addresses
        if( !unique[key] ) {
            bool valid = verifierContract.verifyTx(a,a_p,b,b_p,c,c_p,h,k,input);
            unique[key] = true;
            solutions[msg.sender].push(Solution({valid:valid,isUsed:false, origin:msg.sender}));
    ...
    function mint(address to, uint256 tokenId) public returns(bool) {
        ...
        for(uint256 s = 0; s < solutions[msg.sender].length; s++ ) {
            if( solutions[msg.sender][s].valid && !solutions[msg.sender][s].isUsed)
             { isValid = true;  
              //Mark as used
               solutions[msg.sender][s].isUsed = true;
               break; 
             }
        }
        if( !isValid ) { 
